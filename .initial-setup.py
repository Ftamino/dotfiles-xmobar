...
class MyWindow1(Gtk.Window):
    def __init__(self):
        super().__init__(title="Initial Setup")
        ...
        frame1 = Gtk.Frame(label="Initial Setup - Display")
        
        grid1 = Gtk.Grid(row_spacing    = 10,
                         column_spacing = 10,
                         column_homogeneous = True)

        button1 = Gtk.Button(label="Configure Displays")
        ...
        button2 = Gtk.Button(label="Next Step")
        button2.set_hexpand(True)
        button2.connect("clicked", self.on_button2_clicked)
        
        button3 = Gtk.Button(label="Stop Setup")
        button3.set_hexpand(True)
        button3.connect("clicked", Gtk.main_quit)
        
        label0 = Gtk.Label(
            label="Tipp: To close a window press Mod Key/Windows Key + Q"
        )
        ...
        grid1.attach(button3, 0, 8, 2, 2)
        grid1.attach(label0, 0, 11 , 2 ,2)
        
        self.add(frame1)
        frame1.add(grid1)

    def on_button2_clicked(self, widget):
        print("User chose: Next")
        win1.hide()
        win2.show_all()
...
class MyWindow2(Gtk.Window):
    def __init__(self):
        ...
        frame4 = Gtk.Frame(label="Initial Setup - Colors")
        ...
        colorBtn12 = Gtk.Button(label="Back")
        colorBtn12.set_hexpand(True)
        colorBtn12.connect("clicked", self.on_colorBtn12_clicked)

        colorBtn14 = Gtk.Button(label="Next")
        colorBtn14.set_hexpand(True)
        colorBtn14.connect("clicked", self.on_colorBtn14_clicked)
        ...
        grid4.attach(colorBtn14, 2, 10, 2, 1)
        grid4.attach(colorBtn13, 0, 11, 4, 1)
    ...
    def on_colorBtn14_clicked(self, widget):
        print("Next")
        win2.hide()
        win3.show_all()
...
class MyWindow3(Gtk.Window):
    def __init__(self):
        ...
        frame3 = Gtk.Frame(label="Initial Setup - Install Programms")

        grid3 = Gtk.Grid(row_spacing    = 10,
                         column_spacing = 10,
                         column_homogeneous = True)

        label0 = Gtk.Label(label="Programm Installer will come soon!")
        label0.set_hexpand(True)
        
        colorBtn1 = Gtk.Button(label="Back")
        colorBtn1.set_hexpand(True)
        colorBtn1.connect("clicked", self.on_colorBtn1_clicked)

        colorBtn2 = Gtk.Button(label="Next")
        colorBtn2.set_hexpand(True)
        colorBtn2.connect("clicked", self.on_colorBtn2_clicked)
    ...
    def on_colorBtn2_clicked(self, widget):
        print("Back")
        win3.hide()
        win4.show_all()
...
class MyWindow4(Gtk.Window):
    def __init__(self):
        ...
        colorBtn2 = Gtk.Button(label="Start using xmonad")
        colorBtn2.set_hexpand(True)
